...
pub fn get_chr_start_end(group : &Group) -> hdf5::Result<(Vec<u32>, Vec<u32>, Vec<u32>)>{
    let chrom  = group.dataset("chrom")?.read_1d::<u32>()?.to_vec(); // Get the bin1_id dataset from m_1
    let start  = group.dataset("start")?.read_1d::<u32>()?.to_vec(); // Get the bin2_id dataset from m_1
    let end = group.dataset("end")?.read_1d::<u32>()?.to_vec(); // Get the count dataset from m_1
    Ok((chrom,start,end))
}
...
pub fn get_hic_matrix(
    ...
    // what's the index of the chromosome 
...
fn main() -> Result<()>{
    ...
    let m_1 = read_cool(&args.m_1)?;
    let m_2 = read_cool(&args.m_2)?;
    let m_1_group = get_bins_group(&m_1,&args.bin_size)?;
    let m_2_group = get_bins_group(&m_2,&args.bin_size)?;
    let contacts_m_1 = get_hic_df_kr(&m_1, &args.bin_size, &m_1_group, &args.chr_x,  &args.chr_y)?;
    let contacts_m_2 = get_hic_df_kr(&m_2, &args.bin_size, &m_2_group, &args.chr_x,  &args.chr_y)?;
    
    let matrix_m_1 = get_hic_matrix_kr(&m_1, &args.bin_size, &m_1_group, &args.chr_x,  &args.chr_y)?;
    let matrix_m_2 = get_hic_matrix_kr(&m_2, &args.bin_size, &m_2_group, &args.chr_x,  &args.chr_y)?;
    //println!("{:?}", matrix_m_1);
    //println!("{:?}", matrix_m_2);

    let contacts_reference = (matrix_m_1 + matrix_m_2);
    //println!("{:?}", contacts_reference);
    
    // Structure: bin1, bin2, count_m_1, count_m_2 dataframe
    let mut joined_df = contacts_m_1.join(
        &contacts_m_2,
        ["bin1", "bin2"],
        ["bin1", "bin2"],
    ...
    let count_left = joined_df.column("count").map_err(|e| hdf5::Error::Internal(format!("Polars column addition error: {}", e)))?.clone();
    let count_right = joined_df.column("count_right").map_err(|e| hdf5::Error::Internal(format!("Polars column addition error: {}", e)))?.clone();
    ...
    // Select only upper triangle, which is bin1 < bin2
    let upper = sorted_df
                .lazy()
    ...
    for t in 0..n_permutations{
        ...
        let m_1_down = downsampling(&contacts_reference,factor_h1_u32);
        //println!("{:?}", m_1_down);
        let m_2_down = downsampling(&contacts_reference,factor_h2_u32);
        println!("{:?}", m_2_down);
        let m_1_down_df = array2dataframe(&m_1_down)?;
        let m_2_down_df =  array2dataframe(&m_2_down)?;
        
        //Join the two  dataframes
        let stats_df = m_1_down_df.join(
            &m_2_down_df,
            ["bin1", "bin2"],
            ["bin1", "bin2"],
            JoinArgs::new(JoinType::Inner),
        ...
        // Cast to Float32 for subtraction
        let h1dc = h1down_count
                            .cast(&DataType::Float32).map_err(|e| hdf5::Error::Internal(format!("Polars cast error: {}", e)))?.clone();
    ...
    // Gather all trial series into all_columns
    ...
    // Compare each trial series to the original differences. Count how many times the trial is greater than the original differences
    // This translates to: the trial difference is more extreme than the observed difference therefore this huge difference is due to chance not biological signal
    ...
    "permutation_test_chr{}_chr{}_binsize_{}.csv",
        &args.chr_x, 
        &args.chr_y,
        &args.bin_size
        );
